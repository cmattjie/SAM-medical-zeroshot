...
for idx, batch in enumerate(tqdm(loader)):
    ...
    for mask_s in masks_split:
        ...
        try:
            ...
            board.add_image(f'error_{mask_loc.split("/")[-1]}/mask', mask, idx, dataformats='NHWC')
            board.add_images(f'error_{mask_loc.split("/")[-1]}/mask_rc', np.expand_dims(masks_rc*1, axis=-1), 0, dataformats='NHWC')
            board.add_images(f'error_{mask_loc.split("/")[-1]}/mask_rs3', np.expand_dims(masks_rs3*1, axis=-1), 0, dataformats='NHWC')
            board.add_images(f'error_{mask_loc.split("/")[-1]}/mask_rs5', np.expand_dims(masks_rs5*1, axis=-1), 0, dataformats='NHWC')
            board.add_images(f'error_{mask_loc.split("/")[-1]}/mask_cp', np.expand_dims(masks_cp*1, axis=-1), 0, dataformats='NHWC')
            board.add_images(f'error_{mask_loc.split("/")[-1]}/mask_bb', np.expand_dims(masks_bb*1, axis=-1), 0, dataformats='NHWC')
            continue
    #lista (5 elementos) de listas (2 elementos) de listas de 3 predições de máscara
    mask_list = [masks_rc, masks_rs3, masks_rs5, masks_cp, masks_bb]
    masks_rc, masks_rs3, masks_rs5, masks_cp, masks_bb = utils.merge_masks(mask_list)
